...
- Added a separate versioned natal-interpretation record that stores three to five synthesized English “Chart at a glance” themes, supporting factor IDs, catalog versions, chart-input provenance, generation method, and explicit birth-time uncertainty. Theme generation runs once per current chart-and-library version with a deterministic fallback and lazy generation for existing users.
...
Apply the natal-interpretation migration, review generated themes and relevance retrieval across representative exact-time and unknown-time charts, and then decide where the user-facing “Chart at a glance” should appear in onboarding. Aspect interpretations and dignity metadata remain deferred.
