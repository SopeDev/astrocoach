...
export default async function PatternsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const patterns = await db.mapItem.findMany({ where: { userId: user.id, kind: "PATTERN" }, orderBy: { createdAt: "desc" } });
  ...
  return (
    <main>
      ...
      <section className="mt-8">
        ...
        <MapItemList initialItems={patterns.map((pattern) => ({ id: pattern.id, kind: pattern.kind, statement: pattern.statement, archivedAt: pattern.archivedAt?.toISOString() ?? null }))} locale={locale} messages={messages.map} />
      </section>
    </main>
  );
}
