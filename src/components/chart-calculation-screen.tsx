...
export function ChartCalculationScreen({ title, description }: { title: string; description: string }) {
  return createPortal(
    <div aria-live="polite" className="fixed inset-0 z-50 min-h-svh overflow-hidden bg-[#070916] text-white" role="status">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_38%,rgba(124,58,237,0.25),transparent_32%),radial-gradient(circle_at_20%_80%,rgba(37,99,235,0.18),transparent_28%)]" />
      ...
      <div aria-hidden="true" className="absolute inset-0">
        ...
        <svg className="absolute inset-0 size-full opacity-45" fill="none" viewBox="0 0 390 844">
          ...
          <g fill="currentColor" className="text-violet-200">
      ...
      <div className="relative flex min-h-svh items-center justify-center px-6 py-16 text-center">
        <div className="max-w-sm">
          <div className="chart-orbit mx-auto flex size-28 items-center justify-center rounded-full border border-violet-300/25">
            <div className="flex size-20 items-center justify-center rounded-full border border-violet-300/35 bg-violet-500/10 shadow-[0_0_60px_rgba(139,92,246,0.35)]">
              <MoonStar aria-hidden="true" className="size-8 text-violet-100" />
            </div>
          </div>
          <p className="mt-8 text-xs font-semibold tracking-[0.28em] text-violet-300 uppercase">AstroCoach</p>
          <h2 className="mt-3 text-balance text-3xl font-semibold tracking-tight">{title}</h2>
          <p className="mx-auto mt-4 max-w-xs text-pretty leading-7 text-slate-300">{description}</p>
          <div aria-hidden="true" className="mx-auto mt-8 flex w-24 gap-2">
            <span className="chart-progress-dot h-1 flex-1 rounded-full bg-violet-400" />
            <span className="chart-progress-dot h-1 flex-1 rounded-full bg-violet-400" />
            <span className="chart-progress-dot h-1 flex-1 rounded-full bg-violet-400" />
          </div>
        </div>
