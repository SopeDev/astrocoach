...
export function BirthDataForm({ locale, messages, defaultDate, defaultTime }: {
  locale: Locale;
  ...
  const [timeKnown, setTimeKnown] = useState(defaultTime !== undefined);
  ...
  return (
    <form action={formAction} className="mt-8 space-y-6" noValidate>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/50">
        <label className="flex min-h-11 cursor-pointer items-center justify-between gap-4" htmlFor="birthTimeKnown">
          <span className="flex items-center gap-2 text-sm font-semibold text-slate-800 dark:text-slate-100">
            <Clock3 aria-hidden="true" className="size-4 text-violet-600 dark:text-violet-300" />
            {messages.timeKnownLabel}
          </span>
          <input checked={timeKnown} className="size-5 cursor-pointer accent-violet-600" id="birthTimeKnown" name="birthTimeKnown" onChange={(event) => setTimeKnown(event.target.checked)} type="checkbox" />
        </label>

        {timeKnown ? (
          <div className="mt-4 border-t border-slate-200 pt-4 dark:border-slate-800">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor="birthTime">{messages.timeLabel}</label>
          ...
          <div className="mt-4 flex gap-3 border-t border-slate-200 pt-4 text-sm leading-6 text-slate-600 dark:border-slate-800 dark:text-slate-300">
