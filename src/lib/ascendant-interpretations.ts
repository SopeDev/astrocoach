import { z } from "zod";
import catalogData from "@/data/astrology/ascendants.json";
...
const entrySchema = z.object({
  ...
  kind: z.literal("ascendant"),
  factors: z.object({ sign: z.enum(ASCENDANT_SIGNS) }).strict(),
...
const catalogSchema = z.object({
  ...
  const expectedIds = new Set(ASCENDANT_SIGNS.map((sign) => `ascendant.${slug(sign)}`));
  const actualIds = new Set<string>();

  for (const [index, entry] of catalog.entries.entries()) {
    const expectedId = `ascendant.${slug(entry.factors.sign)}`;
...
export type AscendantSign = typeof ASCENDANT_SIGNS[number];
export type AscendantInterpretation = z.infer<typeof entrySchema>;

export const ascendantInterpretationCatalog = catalogSchema.parse(catalogData);

const interpretationsById = new Map(
  ascendantInterpretationCatalog.entries.map((entry) => [entry.id, entry]),
);

export function getAscendantInterpretation(
  sign: AscendantSign,
): AscendantInterpretation {
  const id = `ascendant.${slug(sign)}`;
  const interpretation = interpretationsById.get(id);
  if (!interpretation) throw new Error(`Missing Ascendant interpretation: ${id}`);
