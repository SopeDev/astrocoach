...
type FocalMapItem = { kind: "PATTERN" | "INSIGHT"; statement: string };
...
export async function generateIntegrateResponse({ locale, focalMapItem, intention, thread, latestMessage, activePractice, recentObservations, astrologyFamiliarity, astrologyStyle }: {
  locale: Locale;
  ...
  const response = await new OpenAI({ apiKey: env.OPENAI_API_KEY }).responses.parse({
    ...
    input: JSON.stringify({ focalMapItem, integrationIntention: intention, activePractice, recentObservations, conversationThread: thread, latestUserMessage: latestMessage, astrologyFamiliarity, astrologyStyle }),
