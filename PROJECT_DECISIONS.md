...
- Natal context is supplied to discovery, EXPLORE, and RECOGNIZE at every astrology communication setting. A global `astrologyStyle` preference controls only how explicitly relevant astrological reasoning appears, while a separate `astrologyFamiliarity` preference controls vocabulary and explanation depth; neither changes the evidence standard or whether the chart is considered.
